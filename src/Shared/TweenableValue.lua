...
function TweenableValue.new<T>(
	...
	-- PRIVATE METHODS
	-------------------------------------------------------------------------------
	local function playTween(playing: Tween): Tween
		tween = playing
	...
	-- When the value changes, so does the instance's property
	function tweenableValue:BindToCallback(callback: (newValue: T, completed: number) -> ())
	...
	function tweenableValue:Tween(newGoal: T, customTweenInfo: TweenInfo?): Tween?
		...
		return playTween(TweenService:Create(valueInstance, customTweenInfo, { Value = goal }))
	end

	-- Cancels any ongoing tweens and tweens to new value in time `length`
	function tweenableValue:Haste(newGoal: T, length: number): Tween?
		if newGoal == goal then
			return
		end
	...
	function tweenableValue:HardReset()
		tweenableValue:Set(initialValue)
	end
